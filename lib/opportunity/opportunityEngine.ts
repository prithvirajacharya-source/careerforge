...
export function scoreOpportunity(input: { profile: OpportunityProfile | null; career: CareerProfile; market: CareerCountryProfile; countryName: string; countryCode: string; countryRows?: CountryIntelligenceRow[]; jobs?: JobPosting[]; liveJobTotal?: number | null }): OpportunityCandidate {
  ...
  const scoreInput: OpportunityScoreInput = {
    ...
    visaRelocationFeasibility: { score: visaRow?.score ?? null, evidence: visaRow ? "Current published SEKUR visa/residency factor." : "Visa/residency evidence is unavailable.", sourceName: visaRow?.source_name, sourceUrl: visaRow?.source_url },
    ...
    languageFit: { score: null, evidence: "No verified career-specific language requirement is available." },
