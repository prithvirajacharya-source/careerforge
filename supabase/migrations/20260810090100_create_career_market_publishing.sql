create table public.career_market_profiles (
  id bigint generated by default as identity primary key,
  career_slug text not null,
  country_slug text not null,
  native_currency text not null,
  salary_low numeric,
  salary_typical numeric,
  salary_high numeric,
  salary_methodology jsonb not null,
  salary_geography text not null,
  observation_period text not null,
  source_name text not null,
  source_url text not null,
  verification_status text not null,
  researched_at timestamptz not null,
  hiring_outlook jsonb not null,
  demand jsonb not null,
  employment_risk jsonb not null,
  education jsonb,
  metric_provenance jsonb not null,
  notes jsonb not null default '[]'::jsonb,
  published_from_run_id bigint not null unique
    references public.career_research_runs(id),
  published_by uuid references auth.users(id) on delete set null,
  published_at timestamptz not null,
  updated_at timestamptz not null default now(),
  unique (career_slug, country_slug)
);

create table public.career_market_profile_versions (
  id bigint generated by default as identity primary key,
  career_slug text not null,
  country_slug text not null,
  ...
  source_run_id bigint references public.career_research_runs(id),
  published_by uuid references auth.users(id) on delete set null,
  published_at timestamptz not null default now()
);

create index career_market_profile_versions_target_idx
  on public.career_market_profile_versions
  (career_slug, country_slug, published_at desc);

alter table public.career_research_runs
  add column published_at timestamptz,
  add column published_by uuid references auth.users(id) on delete set null,
  add column publication_version_id bigint
    references public.career_market_profile_versions(id);

alter table public.career_market_profiles enable row level security;
alter table public.career_market_profile_versions enable row level security;

create policy "Published career market profiles are publicly readable"
  on public.career_market_profiles for select
  to anon, authenticated
  using (true);

create policy "Admins can read career market publication history"
  on public.career_market_profile_versions for select
  to authenticated
  using ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin');

create or replace function public.protect_career_market_profile_version()
returns trigger
language plpgsql
security invoker
...
as $$
begin
  raise exception 'Career market publication history is append-only';
end;
$$;

create trigger protect_career_market_profile_version
before update or delete on public.career_market_profile_versions
for each row execute function public.protect_career_market_profile_version();
...
begin
  ...
  if old.status = 'pending_review' then
    if new.status not in ('approved', 'rejected')
      ...
      or new.publication_version_id is distinct from old.publication_version_id then
      raise exception 'Invalid career research review transition';
    end if;
    return new;
  end if;
  ...
  if old.status = 'approved'
    ...
    and old.publication_version_id is null
    and new.published_at is not null
    and new.published_by is not null
    and new.publication_version_id is not null
    and new.reviewed_by is not distinct from old.reviewed_by
    and new.reviewed_at is not distinct from old.reviewed_at
    and new.review_notes is not distinct from old.review_notes then
    return new;
  end if;

  raise exception 'Career research run has already been reviewed or published';
end;
$$;
...
begin
  ...
  if v_run.career_slug <> 'mechanical-engineer' or v_run.country_slug <> 'sweden' then
    raise exception 'Publishing v1 supports only Mechanical Engineer x Sweden';
  end if;
...
comment on function public.publish_career_market_research(bigint, jsonb) is
  'Atomically validates and publishes an approved Mechanical Engineer x Sweden research run with append-only version history.';
