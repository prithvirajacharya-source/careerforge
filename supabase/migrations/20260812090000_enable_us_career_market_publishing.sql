...
begin
  ...
  if v_run.country_slug not in ('sweden', 'united-states')
    ...
    raise exception 'Publishing supports only enabled Sweden and United States career research targets';
  end if;
  ...
  if (v_run.country_slug = 'sweden' and v_salary ->> 'sourceCurrency' <> 'SEK')
    or (v_run.country_slug = 'united-states' and v_salary ->> 'sourceCurrency' <> 'USD') then
    raise exception 'Candidate native currency does not match the selected labour market';
  end if;
...
comment on function public.publish_career_market_research(bigint, jsonb) is
  'Atomically validates and publishes an approved supported Sweden or United States career research run with append-only version history.';
