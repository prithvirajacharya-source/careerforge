  ...
  return (
    <main className="sekur-discovery min-h-screen">
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-center"><div><p className="product-eyebrow">Career markets</p>
          <h1 className="mt-4 max-w-4xl text-5xl font-black tracking-tight md:text-6xl">Where can your career perform best?</h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-400">Compare markets through the factors that shape a career: opportunity, salary, demand, cost, language, relocation and safety.</p>
        </div><CareerPathArt variant="city" className="hidden w-full lg:block" /></div>
            ...
            <section key={region} className="mt-12">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {regionCountries.map((country) => (
                  <Link key={country.slug} href={`/countries/${country.slug}`} className="group rounded-xl border border-slate-200 bg-white p-5 transition hover:border-blue-300 hover:shadow-sm">
