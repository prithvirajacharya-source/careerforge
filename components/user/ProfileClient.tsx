...
export default function ProfileClient({ user, careers, countries }: { user: User; careers: Array<{ slug: string; title: string }>; countries: Array<{ slug: string; name: string; currency: string }> }) {
  ...
  useEffect(() => {
    supabase.from("user_career_profiles").select("*").eq("user_id", user.id).maybeSingle().then(({ data, error }) => {
      ...
      if (data) {
        ...
        setProfile(loaded); setSkillsText(loaded.skills.join(", ")); setLanguagesText((loaded.languages ?? []).join(", "));
      }
      setLoading(false);
    });
  }, [user.id]);
  ...
  async function save() {
    ...
    try {
      const next = validateUserCareerProfile({ ...profile, skills: normalizeStringList(skillsText.split(","), 100), languages: normalizeStringList(languagesText.split(","), 20) });
      const { error } = await supabase.from("user_career_profiles").upsert({ user_id: user.id, current_country: next.currentCountry, target_countries: next.targetCountries, current_career: next.currentCareer, years_experience: next.yearsExperience, education_level: next.educationLevel, education_field: next.educationField, skills: next.skills, languages: next.languages ?? [], citizenship_region: next.citizenshipRegion, work_authorization_status: next.workAuthorizationStatus ?? "unknown", desired_salary: next.desiredSalary, desired_salary_currency: next.desiredSalaryCurrency, remote_preference: next.remotePreference, relocation_willingness: next.relocationWillingness, career_goals: next.careerGoals, updated_at: new Date().toISOString() });
  ...
  return <section className="glass-panel mx-auto max-w-4xl rounded-3xl border p-6 sm:p-8">
    ...
    <div className="mt-5" aria-label={`Profile completeness ${completeness.score}%`}><div className="flex justify-between text-sm"><span className="font-bold">Profile usefulness</span><span>{completeness.score}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-emerald-300" style={{ width: `${completeness.score}%` }} /></div>{completeness.recommendation && <p className="mt-2 text-sm text-slate-400">{completeness.recommendation}</p>}</div>
