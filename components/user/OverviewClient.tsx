...
export default function OverviewClient({ user }: { user: User }) {
  ...
  return <div>
    <header className="grid gap-6 border-b border-slate-200 pb-7 sm:grid-cols-[1fr_260px] sm:items-center"><div><p className="product-eyebrow">Career status</p><h1 className="mt-2 text-4xl font-bold">Overview</h1><p className="mt-2 text-slate-600">Your strongest signal and the next action worth taking.</p></div><CareerPathArt variant="path" className="hidden w-full sm:block" /></header>
    ...
    <section className="border-b border-slate-200 py-7"><h2 className="text-lg font-bold">Other opportunities</h2>{ranked.length > 1 ? <div className="mt-3 divide-y divide-slate-200">{ranked.slice(1, 4).map(item => <Link key={`${item.careerSlug}:${item.countrySlug}`} href={`/careers/${item.careerSlug}?country=${item.countrySlug}`} className="grid gap-2 py-3 sm:grid-cols-[1fr_auto_auto] sm:items-center"><span>{item.careerName} · {item.countryName}</span><span className="text-sm text-slate-600">{item.evidenceCoverage}% coverage</span><strong>{item.opportunityScore}</strong></Link>)}</div> : <p className="mt-3 text-sm text-slate-600">No additional supported opportunities are available yet.</p>}</section>

    <section className="border-b border-slate-200 py-7"><div className="flex items-center justify-between"><h2 className="text-lg font-bold">Saved opportunities</h2><Link href="/saved" className="text-sm font-bold text-blue-700">View saved →</Link></div>{saved.length ? <div className="mt-3 divide-y divide-slate-200">{saved.map(item => <Link key={item.id} href={item.item_type === "country" ? `/countries/${item.country_slug}` : `/careers/${item.career_slug}${item.country_slug ? `?country=${item.country_slug}` : ""}`} className="flex items-center justify-between py-3"><span>{item.item_type === "country" ? readable(item.country_slug) : `${readable(item.career_slug)}${item.country_slug ? ` · ${readable(item.country_slug)}` : ""}`}</span><span className="text-blue-700">View →</span></Link>)}</div> : <p className="mt-3 text-sm text-slate-600">You have not saved any opportunities yet.</p>}</section>
    <aside className="status-info mt-6 rounded-lg px-4 py-3 text-sm">Scores reflect available evidence. Coverage and confidence remain visible, and missing evidence is never replaced with assumptions.</aside>
  </div>;
}
