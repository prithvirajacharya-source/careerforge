...
export default function AlertPreferencesControl({ careerSlug, countrySlug }: { careerSlug: string; countrySlug: string }) {
  ...
  return <div className="relative"><button type="button" onClick={() => setOpen(!open)} className={`product-button text-xs ${active ? "status-positive" : "product-button-secondary"}`}>{active ? "Alerts on" : "Configure alerts"}</button>{open && <div className="absolute left-0 z-20 mt-2 w-72 rounded-xl border border-slate-200 bg-white p-4 shadow-lg"><div className="font-bold">Alert preferences</div><p className="mt-1 text-xs text-slate-600">Stored for future delivery. No email, SMS or push is sent.</p><div className="mt-3 space-y-2">{ALERT_TYPES.map(type => <label key={type} className="flex gap-2 text-xs"><input type="checkbox" checked={types.includes(type)} onChange={event => setTypes(event.target.checked ? [...types, type] : types.filter(item => item !== type))} />{labels[type]}</label>)}</div><div className="mt-4 flex gap-2"><button type="button" onClick={() => save(true)} className="product-button product-button-primary text-xs">Enable</button><button type="button" onClick={() => save(false)} className="product-button product-button-secondary text-xs">Turn off</button></div>{message && <p className="mt-3 text-xs text-slate-600">{message}</p>}</div>}</div>;
}
