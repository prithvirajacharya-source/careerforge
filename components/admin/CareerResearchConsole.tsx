...
type SourceHealth = { status: string; last_successful_fetch?: string | null; last_failure?: string | null; last_failure_reason?: string | null; consecutive_failures: number; format_drift_detected: boolean; expected_refresh_days?: number | null; next_expected_refresh?: string | null; stale?: boolean };
  ...
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
      <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-6 sm:p-8">
        ...
        {sourceHealth && <div className="mt-3 grid gap-3 rounded-xl border border-white/10 bg-black/15 p-4 text-xs sm:grid-cols-4"><div><div className="font-bold uppercase tracking-wider text-slate-500">Source status</div><div className="mt-1 font-black text-white">{sourceHealth.stale ? "stale" : sourceHealth.status}</div></div><div><div className="font-bold uppercase tracking-wider text-slate-500">Last success</div><div className="mt-1 text-slate-300">{sourceHealth.last_successful_fetch ? new Date(sourceHealth.last_successful_fetch).toLocaleString() : "Not recorded"}</div></div><div><div className="font-bold uppercase tracking-wider text-slate-500">Next expected refresh</div><div className="mt-1 text-slate-300">{sourceHealth.next_expected_refresh ? new Date(sourceHealth.next_expected_refresh).toLocaleDateString() : "Not scheduled"}</div></div><div><div className="font-bold uppercase tracking-wider text-slate-500">Failures</div><div className="mt-1 text-slate-300">{sourceHealth.consecutive_failures}{sourceHealth.format_drift_detected ? " · possible format drift" : ""}</div></div>{sourceHealth.last_failure_reason && <div className="sm:col-span-4"><div className="font-bold uppercase tracking-wider text-slate-500">Last failure</div><div className="mt-1 text-red-200">{sourceHealth.last_failure_reason}</div></div>}</div>}
        ...
        {candidate && target ? <div className="mt-8">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">{(["low", "typical", "high"] as const).map((key) => <div key={key} className="rounded-2xl border border-white/10 bg-[#091426] p-5"><div className="text-xs font-black uppercase tracking-wider text-slate-500">{key}</div><div className="mt-2 text-xl font-black">{formatMoney(candidate.salary[key].value, target.nativeCurrency)} / {(candidate.salary.period ?? "annual") === "annual" ? "year" : (candidate.salary.period ?? "annual").replace("ly", "")}</div><div className="mt-2 text-xs text-slate-500">Live: {formatMoney(liveSalary?.[key], target.nativeCurrency)}</div></div>)}</div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="font-bold">Hiring outlook</div>{candidate.outlookEvidence?.value ? <div className="mt-2 text-sm leading-6 text-slate-300"><strong>{candidate.outlookEvidence.value.projectedGrowthPercent}%</strong> projected growth · {candidate.outlookEvidence.value.annualOpenings.toLocaleString()} annual openings<br /><span className="text-slate-500">{candidate.outlookEvidence.value.forecastPeriod} · <a className="text-blue-300 hover:underline" href={candidate.outlookEvidence.provenance?.sourceUrl} target="_blank" rel="noreferrer">official source</a></span></div> : <div className="mt-1 text-sm text-slate-500">Unavailable — no evidence inferred</div>}</div>
            <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="font-bold">Education / licensing</div>{candidate.education.value ? <div className="mt-2 text-sm leading-6 text-slate-300"><strong>{candidate.education.value.typicalEducation}</strong><br /><span className="text-slate-500">{candidate.education.value.regulatedProfessionStatus?.replaceAll("-", " ") ?? "Regulation unavailable"}{candidate.education.value.licensingRequirements?.length ? ` · ${candidate.education.value.licensingRequirements.join(" ")}` : ""}</span><br /><a className="text-blue-300 hover:underline" href={candidate.education.provenance?.sourceUrl} target="_blank" rel="noreferrer">Qualification source</a></div> : <div className="mt-1 text-sm text-slate-500">Unavailable — no evidence inferred</div>}</div>
            <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="font-bold">Demand</div><div className="mt-1 text-sm text-slate-500">Unavailable — no qualitative label inferred from outlook data</div></div>
            <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="font-bold">Employment risk</div><div className="mt-1 text-sm text-slate-500">Unavailable — no evidence inferred</div></div>
          </div>
