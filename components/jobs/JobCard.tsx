...
export default function JobCard({ job, profile }: { job: JobPosting; profile?: JobMatchProfile | null }) {
  ...
  return <article className="rounded-3xl bg-white/[0.035] p-6 shadow-[0_16px_40px_rgba(0,0,0,0.12)]">
    ...
    {match?.score !== null && match && <div className="mt-5 rounded-2xl bg-blue-300/[0.055] p-4"><div className="font-bold text-blue-200">{match.score}% profile match</div><div className="mt-1 text-xs capitalize text-slate-500">{match.confidence} confidence · {match.evidenceCoverage}% evidence coverage</div><div className="mt-2 text-sm text-slate-400">{match.matchedSkills.slice(0, 3).map((skill) => `${skill} ✓`).join(" · ") || "Based on your career and preferences"}</div>{match.negativeFactors.slice(0, 2).map((factor) => <p key={factor} className="mt-2 text-sm text-slate-300">{factor}</p>)}{learning.length > 0 && <details className="mt-3"><summary className="cursor-pointer text-sm font-semibold text-blue-300">Learning suggestions</summary>{learning.map(({ resource, matchedSkills }) => <a key={resource.id} href={resource.url} target="_blank" rel="noopener noreferrer" className="mt-2 block text-sm text-slate-300 hover:text-emerald-200">Learn {matchedSkills.join(", ")} with {resource.provider} →</a>)}</details>}</div>}
